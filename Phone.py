...
def addcontact():
    ...
    while True:  
        num = input("Какой четырехзначный номер телефона у человека? (допускаются только цифры): ")  
        if not num.isdigit():  
            print("Пожалуйста, вводите только цифры")  
            continue  
        elif len(num) != 4:  
            print("Введите 4-значный номер телефона без запятых, пробелов и знаков препинания.")  
            continue  
        else:  
            break  
